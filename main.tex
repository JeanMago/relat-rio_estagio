...
\titulo{Desenvolvimento de um Hub Administrativo Centralizado para Gestão de Ecossistema SaaS: Projeto Oficina Master}
\autor{Jean Kronabuer de Moura}
\local{Ijuí/RS}
\data{2026}
\orientador{Prof(a). Fabricia Carneiro Roos Frantz}
\instituicao{%
...
\professordabanca{Prof(a). Fabricia Carneiro Roos Frantz}
\datadabanca{XX}{Junho}{2026}
...
\preambulo{Relatório apresentado ao curso de Ciência da Computação da Universidade Regional do Noroeste do Estado do Rio Grande do Sul - UNIJUÍ, como requisito para a obtenção do grau de Bacharel em Ciência da Computação}

% --- Resumo e Abstract ---
\resumoemportugues{%
    Este relatório documenta as atividades realizadas durante o estágio supervisionado focado no desenvolvimento do projeto Oficina Master, um hub administrativo centralizado para um ecossistema de gestão de oficinas mecânicas (SaaS). O projeto teve como objetivo principal solucionar a complexidade da gestão manual de múltiplos tenants e instâncias de banco de dados, além de centralizar o controle de licenciamento e pagamentos. A solução foi desenvolvida utilizando o framework Laravel 12 no backend, integrando-se via API REST com um frontend moderno construído em Vue.js 3 com Pinia. Entre as funcionalidades implementadas, destacam-se a gestão automatizada de migrações de banco de dados, propagação de tabelas tributárias (IBPT), integração com checkout do Mercado Pago e um dashboard administrativo para monitoramento de transações. Os resultados obtidos demonstram uma redução significativa no tempo de suporte técnico e na manutenção do ecossistema, consolidando conhecimentos práticos em arquitetura de software, segurança de dados e integração de serviços terceiros.
}
% Instrução: Use letras minúsculas para as palavras-chave, separe-as com ponto e vírgula (;) e encerre a última com um ponto (.)
\palavraschaveemportugues{laravel; vue.js; saas; gestão de frotas; api rest.}
...
Agradeço à equipe da FK Oficina pela oportunidade e pelo acolhimento durante este período de estágio. Um agradecimento especial ao meu supervisor Eduardo Froner, por compartilhar seus conhecimentos técnicos e me guiar nos desafios do desenvolvimento do projeto Oficina Master. Agradeço também aos meus professores da UNIJUÍ, cujos ensinamentos foram fundamentais para a realização deste trabalho.
